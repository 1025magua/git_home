...
class TestGoodsPromote(unittest.TestCase):
    ...
    def test_goods_promote(self, case):
        ...
        if case.case_id == 19:  # 商品编号不存在的场景
            self.db.cud('delete from tb_goods where goodsId = %s', (eval(case.case_data)["goodsId"]))
        if case.case_id == 18:  # 商品已开启促销的场景
            res = self.db.find_one(
                'select goodsId from tb_goods where isPromote = 0 and isOnSale = 0 and goodsId != %s order by rand()',
                (get_ini_data('goodsId1', 'goodsId'),))
